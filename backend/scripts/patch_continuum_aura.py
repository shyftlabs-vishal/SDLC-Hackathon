#!/usr/bin/env python3
"""Apply Aura gateway model-pin fix to the installed shyftlabs-continuum package.
...
def _gateway_provider_path() -> Path | None:
    try:
        import continuum.llm.providers.gateway_provider as mod
    except ImportError:
        return None
...
def main() -> int:
    target = _gateway_provider_path()
    if target is None:
        print("shyftlabs-continuum not installed — skip Aura patch", file=sys.stderr)
        return 0
    ...
    print(f"Applied Aura gateway model-pin patch to {target}")
    return 0
